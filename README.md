...
We now construct the diffusion operator. To do so, we apply the composite operator $A (2\ket{0...0}\bra{0...0} - I) A^{\dag}$, where $A$ is the entire set of operations that we used to go from the $(n+d)$-bit vacuum state $\ket{0...0}$ to the entangled superposition upon which we applied the oracle operator.
